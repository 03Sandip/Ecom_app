...
// Add Category------------------------------------------------------------------
  addCategory() async {
    try {
      ...
      print(e);
      SnackBarHelper.showErrorSnackBar('An error accured: $e');
      rethrow;
      // Handle exceptions
    }
  }
    ...
    try {
      ...
      if (response.isOk) {
        ...
        if (apiResponse.success == true) {
          ...
          SnackBarHelper.showErrorSnackBar(
              'Failed to add category: ${apiResponse.message}'); // Handle unsuccessful API response
        }
      } else {
        SnackBarHelper.showErrorSnackBar(
            'Error ${response.body?['message'] ?? response.statusText}'); // Handle unsuccessful HTTP response
      }
    } catch (e) {
      print(e);
